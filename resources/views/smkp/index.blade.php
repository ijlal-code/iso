...
@section('content')
    ...
    {{-- ========================================================= --}}
    {{-- FILTER & MONITORING (KHUSUS AUDITOR) --}}
    {{-- ========================================================= --}}
    {{-- HANYA MUNCUL JIKA TIDAK ADA SUB-FOLDER LAGI ($folders->count() == 0) --}}
    
    @if(Auth::user()->role === 'Auditor' && $folders->count() == 0)
        <div class="card border-0 shadow-sm bg-light mb-4 mt-4">
            <div class="card-body p-3">
                <form action="{{ url()->current() }}" method="GET" class="row g-2 align-items-center">
                    <div class="col-md-12 mb-2">
                        <small class="fw-bold text-muted"><i class="bi bi-funnel-fill"></i> Filter Dokumen Unit:</small>
                    </div>
                    <div class="col-md-4">
                        <select name="unit" class="form-select" onchange="this.form.submit()">
                            <option value="">- Semua Unit / Role -</option>
                            @php
                                $roles = ['KTT', 'Pengelola Sistem', 'Audit Internal', 'Pengelola Risiko', 'Pengelola Legal', 'Pengelola K3 & Lingk.', 'Pengel. SDM & Diklat', 'Pengawas Operasional', 'Bag. K3 & KO Pertamb.', 'PJO', 'Pengawas Oper. PJO', 'Pengawas Teknik PJO', 'Bag. K3 & KO PJO'];
                            @endphp
                            @foreach($roles as $role)
                                <option value="{{ $role }}" {{ request('unit') == $role ? 'selected' : '' }}>{{ $role }}</option>
                            @endforeach
                             {{-- Opsi Manual Unit 1-5 --}}
                             <option value="Unit 1" {{ request('unit') == 'Unit 1' ? 'selected' : '' }}>Unit 1</option>
                             <option value="Unit 2" {{ request('unit') == 'Unit 2' ? 'selected' : '' }}>Unit 2</option>
                             <option value="Unit 3" {{ request('unit') == 'Unit 3' ? 'selected' : '' }}>Unit 3</option>
                             <option value="Unit 4" {{ request('unit') == 'Unit 4' ? 'selected' : '' }}>Unit 4</option>
                             <option value="Unit 5" {{ request('unit') == 'Unit 5' ? 'selected' : '' }}>Unit 5</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        <input type="text" name="q" class="form-control" placeholder="Cari nama dokumen..." value="{{ request('q') }}">
                    </div>
                    <div class="col-md-2">
    ...
    {{-- ========================================================= --}}
    {{-- DAFTAR FILE (MUNCUL UNTUK SEMUA ROLE) --}}
    {{-- ========================================================= --}}
    ...
    @if($files->count() > 0)
        <div class="d-flex align-items-center mb-3 mt-4">
            <h6 class="text-secondary fw-bold m-0 border-bottom border-secondary border-2 pb-1 pe-3">
                ...
                @if(request('unit'))
                    <span class="badge bg-danger ms-2">Filter: {{ request('unit') }}</span>
                @endif
            </h6>
        </div>
                        ...
                        @endphp
                            ...
                            <div class="flex-grow-1">
                                ...
                                <div class="small text-muted">
                                    {{ strtoupper($ext) }} &bull; 
                                    <strong class="text-dark"><i class="bi bi-person me-1"></i> {{ $file->user->name ?? '-' }} ({{ $file->user->role ?? 'User' }})</strong> &bull;
    ...
    @elseif($folders->count() == 0 && $files->count() == 0)
        <div class="text-center py-5">
            ...
            @if(request('unit'))
                <br><span class="text-danger small">Tidak ada dokumen dari unit {{ request('unit') }} disini.</span>
            @endif
        </div>
    @endif
