...
@section('content')
    ...
    <style>
        .list-group-item { transition: all 0.2s ease-in-out; border-left: 4px solid transparent; }
        .list-group-item:hover { background-color: #f9fafe; border-left: 4px solid #ffc107; transform: translateX(4px); z-index: 10; }
        .folder-icon { transition: transform 0.2s; }
        .list-group-item:hover .folder-icon { transform: scale(1.15); }
        .custom-dropdown-menu { border: 0; box-shadow: 0 0.5rem 1.5rem rgba(0, 0, 0, 0.15); border-radius: 0.5rem; padding: 0.5rem; min-width: 200px; z-index: 1000; }
        .custom-dropdown-menu .dropdown-item { border-radius: 4px; padding: 8px 12px; font-weight: 500; color: #333; transition: background 0.2s; }
        ...
        .unit-header { background-color: #f8f9fa; border-left: 5px solid #dc3545; }
        .nav-tabs .nav-link { color: #6c757d; border: none; border-bottom: 3px solid transparent; }
        .nav-tabs .nav-link:hover { color: #dc3545; border-color: transparent; }
        .nav-tabs .nav-link.active { color: #dc3545; border-bottom: 3px solid #dc3545; font-weight: bold; }
        .nav-tabs .nav-link#panduan-tab.active { color: #198754; border-bottom: 3px solid #198754; }
    </style>
        ...
        <div class="d-flex gap-2">
            @php
                ...
                if (Auth::user()->role === 'Auditor') {
                    $canUpload = true;
                } elseif (!$currentFolder || ($currentFolder && $currentFolder->type !== 'panduan')) {
                    $canUpload = true; 
                }
            @endphp
    ...
    @if(!$currentFolder)
        ...
        <div class="tab-content" id="rootTabsContent">
            ...
            <div class="tab-pane fade show active" id="utama" role="tabpanel">
                ...
                @else
                    <div class="text-center py-4 text-muted small border rounded bg-light mb-4">Belum ada folder utama.</div>
                @endif
            </div>
            ...
            <div class="tab-pane fade" id="panduan" role="tabpanel">
                <div class="alert alert-info border-0 shadow-sm small py-2">
                    <i class="bi bi-info-circle-fill me-2"></i> Area ini berisi dokumen panduan resmi. 
                    @if(Auth::user()->role !== 'Auditor') (Akses: Read-Only / Hanya Lihat) @endif
                </div>
                @if(Auth::user()->role === 'Auditor')
                    <div class="d-flex justify-content-end mb-2">
                ...
                @else
                    <div class="text-center py-4 text-muted small border rounded bg-light mb-4">Belum ada folder panduan.</div>
                @endif
            </div>
        </div>
    ...
    {{-- ========================================================= --}}
    {{-- TAMPILAN PENCARIAN & MONITORING (KHUSUS AUDITOR & END-FOLDER) --}}
    {{-- ========================================================= --}}
                ...
                <form action="{{ url()->current() }}" method="GET" class="row g-2 align-items-center">
                    <div class="col-md-4">
                        <select name="unit" class="form-select" onchange="this.form.submit()">
                            <option value="">- Filter Semua Unit -</option>
                            @php
                    ...
                    <div class="col-md-6"><input type="text" name="q" class="form-control" placeholder="Cari dokumen / uploader..." value="{{ request('q') }}"></div>
                    <div class="col-md-2"><button type="submit" class="btn btn-danger w-100"><i class="bi bi-search"></i> Cari</button></div>
                </form>
            </div>
        </div>

        {{-- Loop Monitoring Unit (Jika ada implementasi table di sini) --}}
        <div class="row">
            @foreach($targetRoles as $roleName)
                {{-- Sisipkan layout tabel monitoring per unit Anda di sini --}}
            @endforeach
        </div>
    @endif
    ...
    {{-- ========================================================= --}}
    {{-- TAMPILAN FILE --}}
    {{-- KELUAR DARI BLOK @else AGAR SELALU MUNCUL --}}
    {{-- ========================================================= --}}
    @if($files->count() > 0)
            ...
            <form id="deleteForm" action="" method="POST" class="w-100">
                ...
                <div class="modal-content rounded-3 border-0 shadow">
                    <div class="modal-body p-4 text-center">
                        ...
                        <p class="text-muted small mb-4">"<span id="deleteName" class="fw-bold"></span>"<br>Tindakan ini tidak dapat dibatalkan.</p>
    ...
    <script>
        // SCRIPT AUTO-FILL NAMA FILE TANPA EKSTENSI
